...
pub use crate::group_multilinear_extension::DenseGroupMultilinearExtension;
...
pub fn fix_variables<G: CurveGroup>(
    ...
    DenseGroupMultilinearExtension::<G>::from_evaluations_slice(
        nv - dim,
        &poly[..(1 << (nv - dim))],
    )
}
...
pub fn evaluate_no_par<G: CurveGroup>(
    poly: &DenseGroupMultilinearExtension<G>,
    point: &[G::ScalarField],
) -> G {
...
fn fix_last_variable_helper<G: CurveGroup>(
    data: &[G],
    nv: usize,
    point: &G::ScalarField,
) -> Vec<G> {
