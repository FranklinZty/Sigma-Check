...
pub mod testing_code {
    ...
    fn test_left_shift() {
        let data = 5;
        let shift_data = 3 << data;
        println!("{:?}", shift_data);
    }

    // #[test]
    // fn test_fix_one_variable_helper() {
    //     let data = vec![1, 2, 3, 4, 5, 6, 7, 8];
    //     let point = 2;
    //     let res = fix_one_variabl_helper(&data, 3, &point);
    //     assert_eq!(res, vec![10, 14, 18, 22, 26, 30, 34, 38]);
    // }

    #[test]
    fn test_fix_variables() {
        let data = vec![1, 2, 3, 4, 5, 6, 7, 8];
        let mle = to_f_vec(data);
        let point = vec![Fr::from(1u64), Fr::from(1u64)];
        let res = fix_variables(
            &DenseMultilinearExtension::from_evaluations_vec(3, mle),
            &point,
        );
        println!("res:{:?}", res.evaluations);
    }
}
