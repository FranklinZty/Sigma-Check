...
mod tests {
    ...
    fn sum_over_hypercube(poly: &VirtualPolynomial<Fr>) -> Fr {
        let mut sum = Fr::zero();
    ...
    #[test]
    fn field_sumcheck_end_to_end() {
        let mut rng = test_rng();
        let poly = sample_field_poly(4, &mut rng);
    ...
    #[test]
    fn field_sumcheck_rejects_wrong_sum() {
        let mut rng = test_rng();
        let poly = sample_field_poly(4, &mut rng);
        let claimed_sum = sum_over_hypercube(&poly);
        let wrong_sum = claimed_sum + Fr::one();

        let mut prover_transcript = <PolyIOP<Fr> as SumCheck<Fr>>::init_transcript();
        let proof = <PolyIOP<Fr> as SumCheck<Fr>>::prove(&poly, &mut prover_transcript).unwrap();

        let mut verifier_transcript = <PolyIOP<Fr> as SumCheck<Fr>>::init_transcript();
        let verify_res = <PolyIOP<Fr> as SumCheck<Fr>>::verify(
            wrong_sum,
            &proof,
            &poly.aux_info,
            &mut verifier_transcript,
