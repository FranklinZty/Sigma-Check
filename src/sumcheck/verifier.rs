...
mod test {
    use super::*;
    use super::{interpolate_uni_poly, u128_factorial, u64_factorial};
    ...
    */
    ...
    fn test_factorial() {
        for i in 0..10 {
            let a: usize = i;
            let res1 = u64_factorial(a);
            let res2 = u128_factorial(a);
            let res3: Fr = field_factorial(a);
            println!(
                "round: {}, \n res1: {}, \n res2: {}, \n res3: {}",
                i, res1, res2, res3
            );
    ...
    fn test_interpolation() {
        let origional = vec![Fr::from(0), Fr::from(0), Fr::from(2)];

        for i in 3..=20 {
            let eval_point = Fr::from(i);
            // println!("eval at point: {}, the result is: {:?}",i, interpolate_uni_poly(&origional, eval_point));
            println!(
                "eval at point: {}, the result is: {:?}",
                i,
                interpolate_uni_poly(&origional, eval_point).unwrap()
            );
        }
    }
}
