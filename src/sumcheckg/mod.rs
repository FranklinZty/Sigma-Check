...
mod tests {
    ...
    fn random_mle<R: Rng>(num_vars: usize, rng: &mut R) -> Arc<DenseMultilinearExtension<Fr>> {
    ...
    fn sample_group_poly<R: Rng>(
        num_vars: usize,
        rng: &mut R,
    ) -> VirtualGroupPolynomial<G1Projective> {
        let mle1 = random_mle(num_vars, rng);
        let mle2 = random_mle(num_vars, rng);
        let mle3 = random_mle(num_vars, rng);
    ...
    fn sum_over_hypercube(poly: &VirtualGroupPolynomial<G1Projective>) -> G1Projective {
    ...
    #[test]
    fn group_sumcheck_end_to_end() {
        let mut rng = ark_std::test_rng();
        let poly = sample_group_poly(4, &mut rng);
        let claimed_sum = sum_over_hypercube(&poly);
        ...
        assert_eq!(proof.point.len(), poly.aux_info.num_variables);
        assert_eq!(proof.proofs.len(), poly.aux_info.num_variables);
    ...
    #[test]
    fn group_sumcheck_rejects_wrong_sum() {
        let mut rng = ark_std::test_rng();
        let poly = sample_group_poly(4, &mut rng);
        let claimed_sum = sum_over_hypercube(&poly);
        let wrong_sum = claimed_sum + G1Projective::generator();
        ...
        let mut verifier_transcript =
            <PolyIOP<Fr> as GroupSumCheck<G1Projective>>::init_transcript();
        let verify_res = <PolyIOP<Fr> as GroupSumCheck<G1Projective>>::verify(
            wrong_sum,
            &proof,
            &poly.aux_info,
            &mut verifier_transcript,
