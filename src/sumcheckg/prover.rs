...
impl<G: CurveGroup> GroupSumCheckProver<G> for IOPProverState<G> {
    ...
    fn prove_round_and_update_state(
        ...
        for (coefficient, products) in &products_list {
            let (_, eval) = (0..1 << (self.poly.aux_info.num_variables - self.round))
                .into_iter()
                .fold(
                    (
                        vec![(G::ScalarField::zero(), G::ScalarField::zero()); products.len()],
                        vec![G::ScalarField::zero(); products.len() + 1],
                    ),
...
fn extrapolate<G: CurveGroup>(
    points: &[G::ScalarField],
