...
mod tests {
    ...
    fn test_matrix_to_mle() {
        ...
        assert_eq!(a_mle.evaluations.len(), 16); // 4x4 matrix, thus 2bit x 2bit, thus 2^4=16 evals
                                                 // println!( "--- a mle look like ---");
                                                 // for point in A_mle.evaluations.iter() {
                                                 //     println!("{:?}", point);
                                                 // }
        ...
        assert_eq!(a_mle.evaluations.len(), 64); // 5x5 matrix, thus 3bit x 3bit, thus 2^6=64 evals
                                                 ...
                                                 //     println!("{:?}", point); //8*8
                                                 // }
                                                 // println!("A_mle.num_vars = {}", A_mle.num_vars); //6
    ...
    fn test_vec_to_mle() {
        ...
        // println!("z = {:?}, len = {}", z, z.len());

        let n_vars = 3;
        let z_mle = vec_to_mle(n_vars, &z);

        // println!("z_mle.len() = {}", z_mle.evaluations.len());
        // println!("{:?}",z_mle.evaluations);
    ...
    fn test_fix_variables() {
        ...
        for (i, y) in bhc.enumerate() {
            ...
            let fix_left = fix_variables(&a_mle, &y); // try espresso fix_variables
                                                      //println!("y = {:?}, expected_fix_left = {:?}, fix_left = {:?}", y, expected_fix_left, fix_left);
            ...
            // println!("y = {:?}, fix_right = {:?}", y, fix_right);
    ...
    //qc test
    #[test]
    fn test_next_power_of_two_and_log2() {
        assert_eq!(4, (3 as usize).next_power_of_two());
        assert_eq!(8, (5 as usize).next_power_of_two());

        assert_eq!(3, log2(8));
        //println!("log2(8) = {}",log2(7));//3
        //println!("log2(12) = {}",log2(12));//4
    }

    #[test]
    fn test_vec() {
        let _v = vec![vec![0; 5]; 3];
    ...
    fn test_flatten_matrix() {
        ...
        let padmatrix = pad_matrix(&a);
        //println!("padmatrix = {:?}",padmatrix);
        let _flattenmatrix: Vec<Fr> = padmatrix.into_iter().flatten().collect();
        //println!("flattenmatrix = {:?}",flattenmatrix);
    }
}
