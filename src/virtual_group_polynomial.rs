...
impl<G: CurveGroup> VirtualGroupPolynomial<G> {
    ...
    pub fn add_mle_list(
        ...
        let mle_list: Vec<Arc<DenseMultilinearExtension<G::ScalarField>>> =
...
pub fn eq_eval<G: CurveGroup>(
    x: &[G::ScalarField],
    y: &[G::ScalarField],
) -> Result<G::ScalarField, ArithErrors> {
    if x.len() != y.len() {
        return Err(ArithErrors::InvalidParameters(
...
fn build_eq_x_r<G: CurveGroup>(
    r: &[G::ScalarField],
) -> Result<Arc<DenseMultilinearExtension<G::ScalarField>>, ArithErrors> {
...
/// over r, which is
///      eq(x,y) = \prod_i=1^num_var (x_i * r_i + (1-x_i)*(1-r_i))
fn build_eq_x_r_vec<G: CurveGroup>(
    r: &[G::ScalarField],
) -> Result<Vec<G::ScalarField>, ArithErrors> {
...
fn build_eq_x_r_helper<G: CurveGroup>(
    r: &[G::ScalarField],
    buf: &mut Vec<G::ScalarField>,
) -> Result<(), ArithErrors> {
    if r.is_empty() {
...
mod test {
    ...
    fn test_virtual_polynomial_additions() -> Result<(), ArithErrors> {
        ...
        for nv in 2..5 {
            for num_products in 2..5 {
                ...
                let (a, _a_sum) = VirtualGroupPolynomial::<G1Projective>::rand(
                    nv,
                    (2, 3),
                    num_products,
                    &mut rng,
                )?;
                let (b, _b_sum) = VirtualGroupPolynomial::<G1Projective>::rand(
                    nv,
                    (2, 3),
                    num_products,
                    &mut rng,
    ...
    fn test_virtual_polynomial_mul_by_mle() -> Result<(), ArithErrors> {
        ...
        for nv in 2..5 {
            for num_products in 2..5 {
                ...
                assert_eq!(a_eval.mul(b_eval), c_eval);
            }
        }
    ...
    // over r, which is
    //      eq(x,y) = \prod_i=1^num_var (x_i * r_i + (1-x_i)*(1-r_i))
    fn build_eq_x_r_for_test<G: CurveGroup>(
        r: &[G::ScalarField],
    ) -> Arc<DenseMultilinearExtension<G::ScalarField>> {
        // we build eq(x,r) from its evaluations
        // we want to evaluate eq(x,r) over x \in {0, 1}^num_vars
        // for example, with num_vars = 4, x is a binary vector of 4, then
        //  0 0 0 0 -> (1-r0)   * (1-r1)    * (1-r2)    * (1-r3)
        ...
        let one_minus_r: Vec<G::ScalarField> =
            r.iter().map(|ri| G::ScalarField::one() - ri).collect();
